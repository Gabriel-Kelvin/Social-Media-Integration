    ...
    private void handleSigninResult(Task<GoogleSignInAccount> result){
        try{
            ...
            if (account != null) {
                ...
                // Save Gmail sign-in data to SharedPreferences
                SharedPreferences settings = getSharedPreferences(GMAIL_LOGIN, 0);
                SharedPreferences.Editor editor = settings.edit();
                editor.putString("gmail_logged", "gmail_logged");
                editor.apply();

                // Save Gmail sign-in data to another SharedPreferences for later use
                sharedPreferences = getSharedPreferences(SHARED_PREFS, MODE_PRIVATE);
                SharedPreferences.Editor prefsEditor = sharedPreferences.edit();
                prefsEditor.putString("Gmail_name", displayName);
                prefsEditor.putString("Gmail_email", email);
                prefsEditor.putString("Gmail_profileUrl", profilePicUrl);
                prefsEditor.apply();

                // Update UI and proceed to the next activity
